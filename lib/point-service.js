...
function get_placename_from_google_maps( lat, lng, location_type ) {
	var _this = this;
	var types;

	if ( 'neighborhood' === location_type ) {
		types = [ 'sublocality', 'neighborhood' ];
	} else {
		types = [ 'locality', 'administrative_area_level_3', 'administrative_area_level_2' ];
	}
	...
	return new Promise( function( resolve, reject ) {
		...
		_this.geocode.reverse( lat, lng, { result_type: types.join( '|' ) }, function( err, data ) {
			if ( err ) {
				...
				log( "< GOOGLE", lat, lng, location_type, the_result.formatted_address );
				resolution[location_type] = {
					name: the_result.formatted_address,
...
function get_point( lat, lng, callback ) {
	...
	if ( 'undefined' === typeof cached || ! this.cache.is_recent( cached.time ) ) {
		Promise.all( [
			...
			_this.get_placename_from_google_maps( lat, lng, 'city' ),
			_this.get_placename_from_google_maps( lat, lng, 'neighborhood' )
		] ).then( function( data ) {
