...
var PointList = React.createClass( {
	render: function() {
		...
		return (
			<table id="list" className={ false !== this.props.selectedPoint ? 'sticky' : '' }>
				<thead>
					<tr>
	...
	render: function() {
		...
		return (
			<div id="map-container">
				<div id="map"></div>
				<span ref="myTemp" className={className.join( ' ' )}>{ this.props.temperature ? Math.round( this.props.temperature ).toString() + '\u00A0°F' : '' }</span>
			</div>
		);
	}
} );
...
var App = React.createClass( {
	...
	render: function() {
		return (
			<div className={this.state.is_scrolled ? "scroll" : ""}>
